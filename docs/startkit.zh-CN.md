...
| `tunnel_mux` | server/client | 是否开启 tunnel 多路复用。`native` 使用内置 mux command；`vless` 和 `vmess` 使用 Xray 兼容 mux.cool 帧；`trojan` 在两端都是 tcptun 时使用 tcptun 私有 mux。 |
...
server 模式下，TCP 和 UDP 出站目标只允许公网 IP。域名目标会在服务端解析，私有、回环、链路本地、组播、CGNAT 和保留网段会在拨号前被拒绝；native UDP 指向非公网目标的数据包会被直接丢弃。
...
- UDP 使用保守规则：内网 UDP 目标直连，其他 UDP 目标走上游。在 client/server 模式下，流量到达隧道服务端后仍会执行仅公网 IP 出站规则。
...
| vless | 支持 | 支持 | 支持 | 支持 | 支持 | VLESS TCP/UDP，REALITY/Vision，mux.cool |
| vmess | 支持 | 支持 | 支持 | 支持 | 不支持 | VMess AEAD TCP/UDP，security none，mux.cool |
| trojan | 支持 | 支持 | tcptun-to-tcptun | 推荐 | 不支持 | Trojan TCP/UDP |

## 配置建议

- 自己控制 client/server 两端，并希望性能和功能优先：优先使用 `native`。
- 需要对接 Xray VLESS REALITY/Vision：使用 `vless`、`raw`、`tunnel_security: reality`。
- 需要对接 Xray VMess AEAD TCP/UDP：使用 `vmess`，并确认对端是 `security: "none"`。
