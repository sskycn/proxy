...
All tunnel protocols support SOCKS5 UDP relay in client/server mode. With `tunnel_mux` enabled, `native` uses its built-in mux command; `vless` and `vmess` use Xray-compatible mux.cool frames; `trojan` uses a tcptun-private mux layer when both ends are tcptun. With mux disabled, compatibility protocols open one compatibility UDP request per UDP target.

Tunnel multiplexing is enabled by default for client/server tunnels. With multiplexing enabled, `tcptun client` keeps a shared tunnel transport connection to `tcptun server`, then opens one logical stream for each proxied TCP connection or UDP relay. `native` uses the built-in mux command; `vless` and `vmess` use Xray-compatible mux.cool frames; `trojan` uses a tcptun-private mux layer when both ends are tcptun. This reduces WebSocket/HTTP/2/HTTP/3 handshakes and works better behind HTTP/CDN infrastructure. Use `--mux=false` or `"tunnel_mux": false` to fall back to one tunnel transport connection per proxied stream.
